...
// 2. 로컬 번들 모드로 바꾸면 웹뷰 origin이 앱 스킴(iOS capacitor://, Android
//    https://localhost)으로 바뀐다. 지금 API 호출은 상대 경로 /api/graphql +
//    credentials:'include'이고 인증이 httpOnly 쿠키라, origin이 달라지면 rewrite도
//    쿠키 전송도 성립하지 않는다 — 인증 전체를 다시 설계해야 한다.
