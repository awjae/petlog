...
- AWS
- CDKTF (TypeScript 기반 IaC) — Infrastructure as Code 도구로 완전히 전환한다.
...
개념을 묻는 질문("왜", "어떻게 처리해", "best practice")에는 **핵심 하나**부터 답한다.
아는 것을 모두 나열하지 않는다.

- 가장 중요한 개념 하나를 구체적인 예로 먼저 설명한다
- 나머지는 사용자가 물으면 그때 꺼낸다
- 답이 섹션 여러 개와 표 여러 개로 길어지고 있으면, 한 번에 너무 많이 주고 있다는 신호다

## 커밋 · PR

커밋이나 PR을 만들 때는 `/pr` 커맨드를 호출했는지와 무관하게
`.claude/commands/pr.md`의 절차를 따른다. "PR 올려줘" 같은 평문 요청도 같다.

특히 성격이 다른 변경(버그 수정 / 리팩토링 / 죽은 코드 제거 / 문서)은 한 PR에
섞지 않는다. 커밋이 여러 개면 각각 독립적으로 머지 가능한지 먼저 판단하고,
독립적이면 브랜치를 나눈다. 애매하면 묻는다.
...
- 기존 코드가 주변 관습과 다르다는 것만으로 버그라고 단정하지 않는다. 그 값이 무엇을 뜻하고 누구의 기준으로 맞아야 하는지 먼저 확인한다 — 차이가 의도된 것일 수 있다.
- 저장소 전반을 분석하기 전에 `git fetch` 후 현재 브랜치가 `origin/main` 기준으로 최신인지 확인한다. 뒤처진 브랜치에서 낸 분석은 이미 없는 코드를 지적한다.
